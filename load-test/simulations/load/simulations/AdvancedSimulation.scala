...
class AdvancedSimulation extends Simulation {
  ...
  private val userIdFeederRandom = Iterator.continually(Map("userIdRandom" -> (Random.nextInt(simUsers - 1) + 1)))
//  private val mailFeeder = Iterator.continually(Map("email" -> (Random.alphanumeric.take(20).mkString + "@foo.com")))
  ...
  private val httpConfAddUser = http("AddUser-API")
    ...
    .body(StringBody("""{"email":"john${userId}@mail.ru","password":"john${userId}","username":"Mr. John Smith ${userId}", "active": false}"""))
    .check(status.is(200))
